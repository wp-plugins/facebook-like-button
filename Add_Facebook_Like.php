...
if($_POST['sub']){
    ...
    if(mysql_num_rows($Query) == 1){
        ...
        $wpdb->query("UPDATE `$table_name` SET type ='".$type."', appid ='".$appid."', pos ='".$pos."', 
                     layout ='".$layout."', color ='".$color."', verb ='".$verb."', face ='".$face."', 
                     width = '".$width."', css = '".$css."' WHERE id = '1'") or die(mysql_error());
        
    }
    
    if(mysql_num_rows($Query) < 1){
       
       $wpdb->query('INSERT INTO `'.$table_name.'` (`type`, `appid`, `pos`, `layout`, `color`, `verb`, `face` ,`width`, `css`)
       VALUES("'.$type.'", "'.$appid.'", "'.$pos.'" , "'.$layout.'" , "'.$color.'" , "'.$verb.'", "'.$face.'", "'.$width.'", "'.$css.'")')  or die(mysql_error());; 
...
$SDK = '<div id="fb-root"></div>
   ...
   $xfbml = '<div class = "'.$CSS.'"  style = "height: 30px"><fb:like href="'.$url.'" layout="'.$Layout.'" show_faces="'.$face.'" width="'.$Width.'" action="'.$verb.'" colorscheme="'.$color.'" /></div><br>';
   $iframe = ' 
   <div class = "'.$CSS.'" style = "height: 30px"><iframe src="http://www.facebook.com/plugins/like.php?href='.$url.'"&layout='.$Layout.'&show_faces='.$face.'&width='.$Width.'&action='.$verb.'&colorscheme='.$color.'" scrolling="no" frameborder="0" allowTransparency="true" style="border:none; overflow:hidden; width:450px; height:px"></iframe></div><br>';
