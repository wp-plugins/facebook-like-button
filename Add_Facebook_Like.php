...
if($_POST['sub']){
    ...
    if(mysql_num_rows($Query) == 1){
        ...
        mysql_query("UPDATE `$table_name` SET type ='".$type."', appid ='".$appid."', pos ='".$pos."', 
    ...
    if(mysql_num_rows($Query) < 1){
       ...
       mysql_query('INSERT INTO `'.$table_name.'` (`type`, `appid`, `pos`, `layout`, `color`, `verb`, `face`)
...
$SDK = '<div id="fb-root"></div>
   ...
   $xfbml = '<fb:like href="'.$url.'" layout="'.$Layout.'" show_faces="'.$face.'" width="450" action="'.$verb.'" colorscheme="'.$color.'" />';
   $iframe = ' 
   <iframe src="http://www.facebook.com/plugins/like.php?href='.$url.'"&layout='.$Layout.'&show_faces='.$face.'&width=450&action='.$verb.'&colorscheme='.$color.'" scrolling="no" frameborder="0" allowTransparency="true" style="border:none; overflow:hidden; width:450px; height:px"></iframe>
   ';
