...
foreach($Names as $Na){ //Get Options Names
    ...
    $xfbml = '<div class = "' . $Value['css'] . '"  style = "height: ' . $Value['height'] . $Value['ht'] .
        '"><fb:like href="' . $url . '" layout="' . $Value['layout'] . '" show_faces="' . $Value['face'] .
        '" width="' . $Value['width'] . '" action="' . $Value['verb'] . '" colorscheme="' . $Value['color'] . '" font="' . get_option("fblikes_font") .
        '" /></div>';
    $iframe = ' 
   <div class = "' . $Value['css'] . '" style = "height: ' . $Value['height'] . $Value['ht'] .
        '"><iframe src="http://www.facebook.com/plugins/like.php?href=' . $url .
        '&layout=' . $Value['layout'] . '&show_faces=' . $Value['face'] . '&width=' . $Value['width'] . '&action=' .
        $Value['verb'] . '&colorscheme=' . $Value['color'] . fblikes_get_url_locale() . '&font=' . get_option("fblikes_font") .
        '" scrolling="no" frameborder="0" allowTransparency="true" style="border:none; overflow:hidden; width:' . $Value['width'] . 'px; height:' . $Value['height'] . $Value['ht'] .'"></iframe></div>';

    if((is_front_page()) && ($Value['home'] == "true") || (is_single()) && ($Value['post'] == "true") || (is_page()) && ($Value['page'] == "true") ){  
	...
	if((is_front_page()) && ($Value['home'] == "") || (is_single()) && ($Value['post'] == "") || (is_page()) && ($Value['page'] == "") ){  
...
foreach($Names as $Na){ //Get Options Names
    ...
    $xfbml = '<div class = "' . $Value['css'] . '"  style = "height: ' . $Value['height'] . $Value['ht'] .
        '"><fb:like href="' . $url . '" layout="count_button" show_faces="' . $Value['face'] .
        '" width="' . $Value['width'] . '" action="' . $Value['verb'] . '" colorscheme="' . $Value['color'] . '" font="' . get_option("fblikes_font") .
        '" /></div>';
    $iframe = ' 
   <div class = "' . $Value['css'] . '" style = "height: ' . $Value['height'] . $Value['ht'] .
        '"><iframe src="http://www.facebook.com/plugins/like.php?href=' . $url .
        '&layout=count_button&show_faces=' . $Value['face'] . '&width=' . $Value['width'] . '&action=' .
        $Value['verb'] . '&colorscheme=' . $Value['color'] . fblikes_get_url_locale() . '&font=' . get_option("fblikes_font") .
        '" scrolling="no" frameborder="0" allowTransparency="true" style="border:none; overflow:hidden; width:' . $Value['width'] .'px; height:' . $Value['height'] . $Value['ht'] . '"></iframe></div>';
